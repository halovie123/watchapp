...
public class MainActivity extends BaseActivity implements SensorEventListener {
    ...
    // ── Fall state (dùng để push health_records) ──────────────────────────────
    private String currentFallState = "no"; // ✅ THÊM MỚI
    ...
    private final BroadcastReceiver bleUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BLEService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                // ✅ PUSH health_records từ MainActivity
    ...
    private void onFallResponse(boolean isFall, double magnitude) {
        if (isFall) {
            ...
            // ✅ Reset về "no" sau 5 giây
            new Handler(getMainLooper()).postDelayed(() -> {
                currentFallState = "no";
            }, 5000);

        } else {
            // ✅ Cập nhật currentFallState → "no"
            currentFallState = "no";
    ...
    // ── Ghi fall_state (setValue = ghi đè, 1 bản ghi duy nhất) ───────────────
    ...
    // ── Ghi health_records (push = thêm mới mỗi lần) ─────────────────────────
